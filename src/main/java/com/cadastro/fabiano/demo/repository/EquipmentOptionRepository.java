...
public interface EquipmentOptionRepository extends JpaRepository<EquipmentOption, Long> {
    ...
    // Busca paginada por trecho do nome (autocomplete). Mostra todas as opcoes.
    ...
    // Busca paginada apenas das opcoes com disponibilidade (estoque > 0).
    @Query("SELECT o FROM EquipmentOption o WHERE o.catalog.id = :catalogId " +
    ...
    // Remove todas as opcoes de um catalogo (usado ao reimportar/editar a planilha).
    @Modifying
    @Query("DELETE FROM EquipmentOption o WHERE o.catalog = :catalog")
    void deleteByCatalog(@Param("catalog") EquipmentCatalog catalog);

    // Reserva atomica: so incrementa se ainda houver disponibilidade.
    @Modifying
